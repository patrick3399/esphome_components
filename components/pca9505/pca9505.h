...
class PCA9505Component : public Component, public i2c::I2CDevice {
 ...
 protected:
  ...
  esphome::i2c::ErrorCode write_pca_registers(uint8_t start_reg, uint8_t *values, uint8_t len,
                                              bool auto_increment = true);

  uint8_t input_state_[PCA9505_NUM_BANKS]{0};  // Stores last read input values for each bank
  uint8_t output_state_[PCA9505_NUM_BANKS]{
      0};  // Stores desired output values for each bank [cite: 75] (defaults to 0x00)
...
class PCA9505GPIOPin : public GPIOPin {
 public:
  ...
  void set_parent(PCA9505Component *parent) {
    parent_ = parent;
  }
