...
Local override of ESPHome's built-in `i2s_audio` component. Adds `output_gain`
support to the speaker platform, which upstream did not expose at the time this
override was written.

## When to Use

Use this component only on devices that need `output_gain` on the speaker (e.g.
...
additional `output_gain` key is available on the `speaker` platform:
...
speaker:
  - platform: i2s_audio
    ...
    dac_type: external
    i2s_dout_pin: GPIO6
    output_gain: 70%   # ← added by this override
```

## Notes

- This override shadows the upstream component by name. When ESPHome upstream adds
  `output_gain` natively, remove this directory and the `external_components` entry.
