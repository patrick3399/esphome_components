...
void I2SAudioSpeakerBase::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Volume multiplier: %.2fx", this->volume_multiplier_);
}
...
#endif  // USE_AUDIO_DAC
  {
    float effective_volume = clamp(volume * this->volume_multiplier_, 0.0f, 8.0f);
    ...
    if (effective_volume >= 1.0f) {
      ...
      this->software_gain_q16_ = static_cast<uint32_t>(effective_volume * static_cast<float>(1UL << 16));
    } else if (effective_volume <= 0.0f) {
...
void I2SAudioSpeakerBase::apply_software_volume_(uint8_t *data, size_t bytes_read) {
  ...
  if (this->software_gain_q16_ == (1UL << 16)) {
    return;
  }

  for (uint32_t i = 0; i < len; ++i) {
    uint8_t *sample_data = &data[i * bytes_per_sample];
    int64_t sample = audio::unpack_audio_sample_to_q31(sample_data, bytes_per_sample);
    sample = (sample * this->software_gain_q16_) / (1UL << 16);
    sample = clamp<int64_t>(sample, INT32_MIN, INT32_MAX);
    audio::pack_q31_as_audio_sample(static_cast<int32_t>(sample), sample_data, bytes_per_sample);
  }
}
