...
_presence_schema = binary_sensor.binary_sensor_schema(
    ...
    {
        # Minimum per-pixel delta above background to count as "hot" (°C)
        cv.Optional(CONF_THRESHOLD, default=3.0): cv.float_range(min=0.5, max=20.0),
        # Minimum number of pixels above threshold required to declare presence
        cv.Optional(CONF_MIN_PIXELS, default=2): cv.int_range(min=1, max=64),
    }
)
...
_hot_spot_schema = binary_sensor.binary_sensor_schema(
    ...
    {
        # Absolute temperature threshold; any pixel at or above this triggers (°C)
        cv.Optional(CONF_THRESHOLD, default=50.0): cv.float_range(min=20.0, max=300.0),
    }
)
...
async def to_code(config):
    ...
    if CONF_PRESENCE in config:
        ...
        cg.add(hub.set_presence_threshold(config[CONF_PRESENCE][CONF_THRESHOLD]))
        cg.add(hub.set_presence_min_pixels(config[CONF_PRESENCE][CONF_MIN_PIXELS]))

    if CONF_HOT_SPOT in config:
        bs = await binary_sensor.new_binary_sensor(config[CONF_HOT_SPOT])
        cg.add(hub.set_hot_spot_sensor(bs))
        cg.add(hub.set_hot_spot_threshold(config[CONF_HOT_SPOT][CONF_THRESHOLD]))
