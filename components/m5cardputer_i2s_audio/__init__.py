...
CONF_I2S_AUDIO = "m5cardputer_i2s_audio"
CONF_I2S_AUDIO_ID = "m5cardputer_i2s_audio_id"
...
i2s_audio_ns = cg.esphome_ns.namespace("m5cardputer_i2s_audio")
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_I2S_PORT): cv.int_range(min=0),
    },
)
...
def _assign_ports() -> None:
    ...
    for config in i2s_configs:
        if CONF_I2S_PORT not in config:
            continue
        config_id = str(config[CONF_ID])
        port = config[CONF_I2S_PORT]
        if port >= max_ports:
            raise cv.Invalid(f"I2S port {port} is not available on {variant}")
        if port in used_ports:
            raise cv.Invalid(f"I2S port {port} is configured more than once")
        data.port_map[config_id] = port
        used_ports.add(port)

    # Assign ports: port 0 parent first (if any), rest get sequential
    next_port = 0
    if port0_parent_id is not None and port0_parent_id not in data.port_map:
        while next_port in used_ports:
            next_port += 1
        if next_port != 0:
            raise cv.Invalid("PDM/ADC microphone requires I2S port 0")
        data.port_map[port0_parent_id] = next_port
        used_ports.add(next_port)
        next_port += 1
    for config in i2s_configs:
        config_id = str(config[CONF_ID])
        if config_id != port0_parent_id and config_id not in data.port_map:
            while next_port in used_ports:
                next_port += 1
