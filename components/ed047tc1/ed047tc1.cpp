...
static void custom_board_init_callback(uint32_t epd_row_width_param, const EpdInitConfig* init_config) {
  ...
  if (self->pwr_pin_)
    self->pwr_pin_->digital_write(false);
  if (self->bst_en_pin_)
    self->bst_en_pin_->digital_write(false);

  const EpdDisplay_t* display_model = epd_get_display();
  if (!display_model) {
    ESP_LOGE(TAG, "board_init_cb: epd_get_display() null.");
    return;
  }
  ...
  for (int i = 0; i < 8; ++i) {
    bus_cfg.data[i] = esphome_pin_to_gpio_num(self->d_pins_[i]);
  }
  ...
  if (self->pclk_pin_ == nullptr && self->xstl_pin_ == nullptr) {
    ESP_LOGE(TAG, "Critical: No valid Pixel Clock source!");
  } else if (self->pclk_pin_ == nullptr) {
    ESP_LOGW(TAG, "Warning: Using XSTL_PIN as PCLK source.");
  }
...
static void custom_board_set_ctrl_callback(epd_ctrl_state_t* state, const epd_ctrl_state_t* mask) {
  if (!ED047TC1Display::instance)
    return;
  ED047TC1Display* self = ED047TC1Display::instance;
  if (mask->ep_sth && self->xstl_pin_)
    self->xstl_pin_->digital_write(state->ep_sth);
  if (mask->ep_stv && self->spv_pin_)
    self->spv_pin_->digital_write(state->ep_stv);
  if (mask->ep_latch_enable && self->xle_pin_)
    self->xle_pin_->digital_write(state->ep_latch_enable);
}
...
static void custom_board_poweron_callback(epd_ctrl_state_t* state) {
  ...
  if (self->pwr_pin_)
    self->pwr_pin_->digital_write(true);
  esphome::delayMicroseconds(200);
  if (self->bst_en_pin_)
    self->bst_en_pin_->digital_write(true);
  esphome::delayMicroseconds(200);
  if (self->spv_pin_)
    self->spv_pin_->digital_write(true);
  if (self->xstl_pin_)
    self->xstl_pin_->digital_write(true);
}
...
static void custom_board_poweroff_callback(epd_ctrl_state_t* state) {
  ...
  esphome::delayMicroseconds(200);
  if (self->spv_pin_)
    self->spv_pin_->digital_write(false);
  if (self->xstl_pin_)
    self->xstl_pin_->digital_write(false);
  if (self->xle_pin_)
    self->xle_pin_->digital_write(false);
  if (self->ckv_pin_)
    self->ckv_pin_->digital_write(false);
  for (int i = 0; i < 8; ++i) {
    if (self->d_pins_[i])
      self->d_pins_[i]->digital_write(false);
  }
}

static float custom_board_get_temperature_callback() {
  return 25.0f;
}
...
void ED047TC1Display::setup() {
  ...
  auto setup_pin = [&](GPIOPin* pin_obj, const char* name) {
    if (pin_obj) {
      ...
      ESP_LOGE(TAG, "%s not configured!", name);
      this->mark_failed();
    }
  };
  ...
  for (int i = 0; i < 8; ++i) {
    char pin_name[10];
    sprintf(pin_name, "D%d pin", i);
    setup_pin(this->d_pins_[i], pin_name);
  }
  if (this->is_failed())
    return;
  ...
  if (epd_hl_get_framebuffer(&this->hl_state_) == nullptr) {
    ESP_LOGE(TAG, "Failed to init epdiy high-level state!");
    this->mark_failed();
    return;
  }
  ...
  if (this->buffer_ == nullptr) {
    ESP_LOGE(TAG, "Could not allocate ESPHome DisplayBuffer!");
    this->mark_failed();
    return;
  }
  memset(this->buffer_, 0xFF, this->esphome_buffer_size_);
  ESP_LOGI(TAG, "ESPHome buffer allocated: %p, size %u.", (void*)this->buffer_,
           (unsigned int)this->esphome_buffer_size_);
...
void ED047TC1Display::update() {
  this->do_update_();
  if (!this->buffer_) {
    ESP_LOGE(TAG, "ESPHome buffer null in update!");
    return;
  }
  if (!epd_hl_get_framebuffer(&this->hl_state_)) {
    ESP_LOGE(TAG, "EPDiy FB null in update!");
    return;
  }
...
void ED047TC1Display::partial_update(int x, int y, int w, int h, enum EpdDrawMode mode) {
  ...
  if (err != EPD_DRAW_SUCCESS) {
    ESP_LOGE(TAG, "partial_update failed: %d", err);
  }
  epd_poweroff();
}

void ED047TC1Display::draw_absolute_pixel_internal(int x, int y, Color color) {
  if (x < 0 || x >= get_width_internal() || y < 0 || y >= get_height_internal() || !this->buffer_)
    return;
  uint32_t r_val = color.r;
...
void ED047TC1Display::dump_config() {
  ...
  LOG_PIN("  PWR Pin: ", pwr_pin_);
  LOG_PIN("  BST_EN Pin: ", bst_en_pin_);
  LOG_PIN("  XSTL_PIN (STH/DE): ", xstl_pin_);
  if (pclk_pin_) {
    LOG_PIN("  PCLK_PIN (LCD Clock): ", pclk_pin_);
  } else {
    ESP_LOGCONFIG(TAG, "  PCLK_PIN: Not configured (XSTL_PIN attempted as PCLK - risky)");
  }
  LOG_PIN("  XLE_PIN (LEH): ", xle_pin_);
  LOG_PIN("  SPV_PIN (STV): ", spv_pin_);
